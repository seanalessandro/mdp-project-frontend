...
import { Layout, Button, Space, Typography, Tag, Card, Popconfirm, message, Row, Col } from 'antd';
import { EditOutlined, MailOutlined, HistoryOutlined, BranchesOutlined, CheckCircleOutlined, CommentOutlined } from '@ant-design/icons';
...
export default function PreviewClient({ documentData, docId }: { documentData: any, docId: string }) {
    ...
    useEffect(() => {
        if (documentData && editor && documentData.content) {
            try {
                const contentJson = JSON.parse(documentData.content);
                editor.commands.setContent(contentJson);
            } catch (e) {
                console.error("Invalid content format:", e);
                editor.commands.setContent("<p>Error displaying content.</p>");
            }
        }
    }, [documentData, editor]);
    ...
    const handleSetStatus = async (newStatus: string) => {
        try {
            ...
            setStatus(newStatus);
            message.success(`Dokumen berhasil ditandai sebagai "${newStatus}"`);
    ...
    return (
        <Content style={{ margin: -24, padding: 24, background: '#fff', borderRadius: 8 }}>
            <div style={{ marginBottom: '24px' }}>
                <a onClick={() => router.push('/dashboard/documents')} style={{ color: '#888' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', borderBottom: '1px solid #f0f0f0', paddingBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '12px' }}>
                    <Space>
                        ...
                        <Button icon={<BranchesOutlined />} shape="round" style={{ display: 'flex', alignItems: 'center', color: '#555' }}>
