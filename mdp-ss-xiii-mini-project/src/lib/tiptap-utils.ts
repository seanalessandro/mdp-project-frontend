...
export const handleImageUpload = async (file: File): Promise<string> => {
  ...
  try {
    // 2. Panggil fungsi api.uploadImage untuk mengirim file ke backend
    const response = await api.uploadImage(file);

    // 3. Pastikan response dari backend memiliki properti 'url'
    if (response && response.url) {
      // 4. Kembalikan URL ke Tiptap untuk ditampilkan di editor
      return response.url;
    } else {
      throw new Error("Respons server tidak menyertakan URL gambar.");
    }
  } catch (error: unknown) {
    console.error("Upload gambar gagal:", error);
    // Melempar error akan memberitahu Tiptap bahwa proses upload gagal
    if (error instanceof Error) {
      throw new Error(`Upload gagal: ${error.message}`);
    }
    throw new Error(`Upload gagal: Terjadi kesalahan yang tidak diketahui`);
  }
}
